...
export function PreferencesModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="glass-panel w-full max-w-4xl h-[620px] rounded-3xl border border-border shadow-2xl flex overflow-hidden text-xs animate-in fade-in zoom-in-95 duration-200">
        
        {/* Left Navigation Sidebar (Linear Settings Style) */}
        <div className="w-60 bg-background/80 border-r border-border/80 p-4 flex flex-col justify-between shrink-0">
          <div>
            {/* Modal Title */}
            <div className="flex items-center justify-between pb-4 border-b border-border/80 mb-4 px-2">
              <span className="font-heading font-bold text-sm text-foreground tracking-tight">Preferências</span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full px-3 py-2.5 rounded-xl text-left flex items-center gap-3 font-medium transition ${
                      isActive
                        ? 'bg-elevated text-primary border border-primary/30'
          ...
          <div className="px-2 pt-4 border-t border-border/80 text-[10px] text-faint font-mono-tech">
            $Agent Platform • Linear UI
          </div>
        </div>

        {/* Right Tab Content Panel */}
        <div className="flex-1 flex flex-col justify-between bg-card/60 p-6 overflow-y-auto custom-scrollbar">
          <div className="flex flex-col gap-6">
            ...
            {activeTab === 'profile' && (
              <div className="flex flex-col gap-5">
                <div className="flex items-center gap-4 p-4 rounded-2xl bg-background/80 border border-border">
                  <img
                    src={formData.avatarUrl}
                    alt={formData.name}
                    className="w-16 h-16 rounded-2xl object-cover border border-primary/40"
                  />
                  <div>
                    <h4 className="font-heading font-bold text-sm text-foreground">{formData.name}</h4>
                    <p className="text-muted-foreground text-xs mt-0.5 font-mono-tech">jppfeitosa@gmail.com</p>
                    <span className="inline-block mt-2 text-[10px] px-2.5 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20 font-mono-tech">
                ...
                <div className="flex flex-col gap-3">
                  <label className="text-foreground font-semibold">Nome de Exibição</label>
                  <input
                    type="text"
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    className="w-full px-4 py-3 rounded-xl bg-background border border-border text-foreground text-xs focus:outline-none focus:border-primary"
                  />
                </div>

                <div className="flex flex-col gap-3">
                  <label className="text-foreground font-semibold">E-mail</label>
                  <input
                    type="email"
                    value="jppfeitosa@gmail.com"
                    disabled
                    className="w-full px-4 py-3 rounded-xl bg-background/60 border border-border text-muted-foreground text-xs opacity-70 cursor-not-allowed font-mono-tech"
                  />
                </div>
              </div>
            )}
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-2">
                    <label className="text-foreground font-semibold">Tamanho de Roupa</label>
                    <select
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-foreground font-semibold">Tamanho de Sapato</label>
                    <input
                      type="text"
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-foreground font-semibold">Teto Máximo de Orçamento (R$)</label>
                  <input
                    type="number"
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-foreground font-semibold">Estilos Preferidos</label>
            ...
            {activeTab === 'byok' && (
              <div className="flex flex-col gap-5">
                <div className="p-4 rounded-2xl bg-background border border-border flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-foreground text-sm">Bring Your Own Key (BYOK)</h4>
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-foreground font-semibold">Provedor de IA Selecionado</label>
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      >
                        ...
                        <span className="text-[10px] text-faint font-mono-tech mt-0.5">{prov.desc}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-foreground font-semibold">Chave de API Personalizada (Opcional)</label>
                  <input
                    type="password"
                    value={customApiKey}
                    onChange={(e) => onApiKeyChange(e.target.value)}
                    placeholder="sk-..."
                    className="w-full px-4 py-3 rounded-xl bg-background border border-border text-foreground text-xs font-mono-tech focus:outline-none focus:border-primary"
                  />
                </div>
              </div>
            )}
                ...
                <div className="flex flex-col gap-3">
                  {[
                    ...
                    return (
                      ...
                      >
                        <div>
                          <span className="font-bold text-foreground text-sm block">{preset.name}</span>
                          <span className="text-xs text-muted-foreground mt-0.5 block">{preset.desc}</span>
                        </div>
                        {isActive && (
            ...
            {activeTab === 'privacy' && (
              <div className="flex flex-col gap-4 text-foreground leading-relaxed">
                <div className="p-4 rounded-2xl bg-background border border-border flex flex-col gap-2">
                  <span className="font-bold text-foreground text-sm">🔒 Proteção & Privacidade do Usuário</span>
                ...
                <div className="p-4 rounded-2xl bg-background border border-border flex flex-col gap-2">
                  <span className="font-bold text-foreground text-sm">📜 Licença Open-Source Comercial (MIT)</span>
                  <p className="text-muted-foreground text-xs">
          ...
          {/* Footer Actions */}
          <div className="pt-4 border-t border-border/80 flex items-center justify-end gap-3 mt-6">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 rounded-xl bg-background border border-border text-foreground hover:text-foreground hover:bg-elevated transition font-medium"
            >
              Cancelar
            </button>
